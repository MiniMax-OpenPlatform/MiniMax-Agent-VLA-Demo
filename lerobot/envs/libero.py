...
class LiberoEnv(gym.Env):
    ...
    def reset(self, seed=None, **kwargs):
        ...
        # Force reset the underlying robosuite environment
        ...
        try:
            ...
            # If step fails (terminated state), reset again
            raw_obs = self._env.reset()
            for _ in range(self.num_steps_wait):
                raw_obs, _, _, _ = self._env.step(get_libero_dummy_action())
    ...
    def step(self, action: np.ndarray) -> tuple[dict[str, Any], float, bool, bool, dict[str, Any]]:
        ...
        # Force clear robosuite's internal termination flag to allow continuous tasks
        if hasattr(self._env, 'env') and hasattr(self._env.env, 'done'):
            self._env.env.done = False
        raw_obs, reward, done, info = self._env.step(action)

        is_success = self._env.check_success()
        # Never terminate - allows continuous multi-step tasks
        terminated = False
        info.update(
            {
                "task": self.task,
                ...
                "success": is_success,  # Also add "success" key for compatibility
            }
        )
        observation = self._format_raw_obs(raw_obs)
        truncated = False
